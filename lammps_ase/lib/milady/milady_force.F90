...
!     Compute forces atom i
      ...
      if (elti.gt.0) then
        ...
        if (snap_order == snap_quadratic) then
            ...
            q_etmp = dot_product( q_local_desc(1:dim_xdesc_quadratic_only), weights(dim_xdesc_linear+1:dim_xdesc_quadratic, ninteractions))
        end if
      ...
      if (snap_order == snap_quadratic) then
            ...
            do id1=1, dim_xdesc
                  do id2 = 1, dim_xdesc
                        indx =dim_xdesc*(id1-1) + id2
                        q_local_desc_deriv(indx,:) = local_desc_deriv(id1,0,:)*local_desc(id2) + local_desc_deriv(id2,0,:)*local_desc(id1)
                  end do
            end do
! those are the forces on the atom i ... the first part ... those in first  r_cut shell
            do ci=1,3
                  f(ci,i) =  f(ci,i) - dot_product(q_local_desc_deriv(1:dim_xdesc_quadratic_only,ci), weights(dim_xdesc_linear+1:dim_xdesc_quadratic, ninteractions)) * fscale
            end do
...
!the other part of forces .... on atom j that was in the list of atom i ... not complete but lammps will do the job to complete tgrough mpi.
            do jn = 1, n_desc_type
                  ...
                  do id1=1, dim_xdesc
                        do id2 = 1, dim_xdesc
                              indx =dim_xdesc*(id1-1) + id2
                              q_local_desc_deriv(indx,:) = local_desc_deriv(id1,jn,:)*local_desc(id2) + local_desc_deriv(id2,jn,:)*local_desc(id1)
                        end do
                  end do

                  do ci=1,3
                        f(ci, j)=f(ci,j) - dot_product(q_local_desc_deriv(1:dim_xdesc_quadratic_only,ci)  , weights(dim_xdesc_linear+1:dim_xdesc_quadratic, ninteractions)  ) * fscale
                  end do
            end do
      end if
...
!put the stress ... one day
        if (vflag_atom.ne.0) then
            ...
            do jn = 1, n_desc_type
                  ...
                  if (snap_order == snap_kernel) then
                        st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 1) = st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 1) + local_desc_kernel_deriv(1:dim_kernel,jn,1)*uu(1)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 2) = st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 2) + local_desc_kernel_deriv(1:dim_kernel,jn,2)*uu(2)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 3) = st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 3) + local_desc_kernel_deriv(1:dim_kernel,jn,3)*uu(3)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 4) = st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 4) + local_desc_kernel_deriv(1:dim_kernel,jn,2)*uu(3)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 5) = st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 5) + local_desc_kernel_deriv(1:dim_kernel,jn,1)*uu(3)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 6) = st_temp(dim_xdesc+1:dim_xdesc_kernel-1 , 6) + local_desc_kernel_deriv(1:dim_kernel,jn,1)*uu(2)/rr
                  end if



                  if (snap_order == snap_quadratic) then
                        st_temp(dim_xdesc+1:dim_xdesc_quadratic-1, 1) = st_temp(dim_xdesc+1:dim_xdesc_quadratic-1,1) + q_local_desc_deriv(dim_xdesc+1:dim_xdesc_quadratic-1,1)*uu(1)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_quadratic-1, 2) = st_temp(dim_xdesc+1:dim_xdesc_quadratic-1,2) + q_local_desc_deriv(dim_xdesc+1:dim_xdesc_quadratic-1,2)*uu(2)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_quadratic-1, 3) = st_temp(dim_xdesc+1:dim_xdesc_quadratic-1,3) + q_local_desc_deriv(dim_xdesc+1:dim_xdesc_quadratic-1,3)*uu(3)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_quadratic-1, 4) = st_temp(dim_xdesc+1:dim_xdesc_quadratic-1,4) + q_local_desc_deriv(dim_xdesc+1:dim_xdesc_quadratic-1,2)*uu(3)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_quadratic-1, 5) = st_temp(dim_xdesc+1:dim_xdesc_quadratic-1,5) + q_local_desc_deriv(dim_xdesc+1:dim_xdesc_quadratic-1,1)*uu(3)/rr
                        st_temp(dim_xdesc+1:dim_xdesc_quadratic-1, 6) = st_temp(dim_xdesc+1:dim_xdesc_quadratic-1,6) + q_local_desc_deriv(dim_xdesc+1:dim_xdesc_quadratic-1,1)*uu(2)/rr
                  end if
            ...
            if (snap_order >= snap_linear) then
                  do ci = 1,6
                        vatom(ci,i) = vatom(ci,i) + dot_product(st_temp(1:dim_xdesc, ci), weights(2:dim_xdesc+1, ninteractions))/3.d0 * fscale
                  enddo
            end if
            ...
            if (snap_order == snap_kernel) then
                  do ci = 1,6
                        vatom(ci,i) = vatom(ci,i) + dot_product(st_temp(dim_xdesc+1:dim_xdesc_kernel-1, ci), weights(dim_xdesc+2:dim_xdesc_kernel, ninteractions))/3.d0 * fscale
                  enddo
            end if

            if (snap_order == snap_quadratic) then
                  do ci = 1,6
                        vatom(ci,i) = vatom(ci,i) + dot_product(st_temp(dim_xdesc+1:dim_xdesc_quadratic-1, ci), weights(dim_xdesc+2:dim_xdesc_quadratic, ninteractions))/3.d0 * fscale
                  enddo
            end if
