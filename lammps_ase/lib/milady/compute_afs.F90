...
do ja=i_start_at,i_final_at
            ...
            do p1=pmin, pmax
               do p2=0,n_cheb
                  ...
                  if (desc_forces) then
                        ...
                        if (weighted) then
                           local_afs_deriv(i_desc2,ia_n,1:3) = local_afs_deriv(i_desc2,  ia_n, 1:3) + factor_ia*factor_ka*term_local_3d(1:3)
                           if (weighted_3ch) local_afs_deriv(i_desc3,ia_n,1:3) = local_afs_deriv(i_desc3, ia_n, 1:3) + factor_ia_3ch*factor_ka_3ch*term_local_3d(1:3)
                        end if
                  end if
                  !
               enddo   !p2 cos
   ...
   if (desc_forces)  then
      ...
      if (weighted) then
         ...
         if (weighted_3ch) local_afs_deriv(2*afs_dim+1:3*afs_dim,0:ia_n,:) = local_afs_deriv(2*afs_dim+1:3*afs_dim,0:ia_n,:)*factor_ja_3ch
      end if
   end if
