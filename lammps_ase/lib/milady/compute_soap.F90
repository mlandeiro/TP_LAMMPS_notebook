   ...
   if (desc_forces) then
      ...
      i_desc = 0
         ...
         if (lsoap_norm) then
           ...
           do ix=1,3
               local_soap_deriv(:,ia,ix)=d_tmp_soap(:,ix) -tmp_soap(:)*DOT_PRODUCT(tmp_soap(:),d_tmp_soap(:,ix))/norm_descriptor**2
           end do
         else
...
use ml_in_lammps_module, ONLY: l_max, n_soap, W_soap, lsoap_diag, soap_dim, nspecies_soap, rb_soap, ns_soap_index, one_pi, alpha_soap, rangml,  debug, atom_sigma_soap, S_factor_matrix
