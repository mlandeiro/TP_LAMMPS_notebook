    ...
    do ja = i_start_at, i_final_at
      ...
      if (desc_forces) then
        do ia = 1, d_n_neigh
          ...
          local_pso3_deriv(1:pow_so3_dim, 0, 1:3) = local_pso3_deriv(1:pow_so3_dim, 0, 1:3) - tmp_pow_so3_deriv_out(1:pow_so3_dim, ia, 1:3)
        end do
        if (weighted) then
          do ia = 1, d_n_neigh
            local_pso3_deriv(pow_so3_dim+1:2*pow_so3_dim,  ia, 1:3) = tmp_pow_so3_deriv_out_w(1:pow_so3_dim, ia, 1:3)!*factor_ja
            local_pso3_deriv(pow_so3_dim+1:2*pow_so3_dim,  0, 1:3) = local_pso3_deriv(pow_so3_dim+1:2*pow_so3_dim, 0, 1:3) - tmp_pow_so3_deriv_out_w(1:pow_so3_dim, ia, 1:3)!*factor_ja
          end do
          if (weighted_3ch) then
            do ia = 1, d_n_neigh
              local_pso3_deriv(2*pow_so3_dim+1:3*pow_so3_dim,  ia, 1:3) = tmp_pow_so3_deriv_out_w_3ch(1:pow_so3_dim, ia, 1:3)!*factor_ja_3ch
              local_pso3_deriv(2*pow_so3_dim+1:3*pow_so3_dim,  0, 1:3) = local_pso3_deriv(2*pow_so3_dim+1:3*pow_so3_dim, 0, 1:3) - tmp_pow_so3_deriv_out_w_3ch(1:pow_so3_dim, ia, 1:3)!*factor_ja_3ch
            end do
          end if
        end if
      end if
