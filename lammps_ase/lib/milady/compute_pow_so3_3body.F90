    ...
    do ja = i_start_at, i_final_at
        ...
        do n2 = ini_rbf_so3, n_rbf_so3
          do l = 0, l_max
            ...
            do m = -l, l
              ...
              do ia = 1, d_n_neigh
                if (desc_forces) tmp_pow_so3_deriv_out(i_count, ia, 1:3) = tmp_pow_so3_deriv_out(i_count, ia, 1:3) + &
              ...
              if (weighted) then
                ...
                do ia = 1, d_n_neigh
                  if (desc_forces) tmp_pow_so3_deriv_out_w(i_count, ia, 1:3) = tmp_pow_so3_deriv_out_w(i_count, ia, 1:3) + &
                ...
                if (weighted_3ch) then
                  ...
                  do ia = 1, d_n_neigh
                    if (desc_forces) tmp_pow_so3_deriv_out_w_3ch(i_count, ia, 1:3) = tmp_pow_so3_deriv_out_w_3ch(i_count, ia, 1:3) + &
      ...
      if (desc_forces) then
        do ia = 1, d_n_neigh
          ...
          local_psb3_deriv(1:pow_so3_dim, 0, 1:3) = local_psb3_deriv(1:pow_so3_dim, 0, 1:3) - tmp_pow_so3_deriv_out(1:pow_so3_dim, ia, 1:3)
        end do
        if (weighted) then
          do ia = 1, d_n_neigh
            local_psb3_deriv(pow_so3_dim+1:2*pow_so3_dim,  ia, 1:3) = tmp_pow_so3_deriv_out_w(1:pow_so3_dim, ia, 1:3)!*factor_ja
            local_psb3_deriv(pow_so3_dim+1:2*pow_so3_dim,  0, 1:3) = local_psb3_deriv(pow_so3_dim+1:2*pow_so3_dim, 0, 1:3) - tmp_pow_so3_deriv_out_w(1:pow_so3_dim, ia, 1:3)!*factor_ja
          end do
          if (weighted_3ch) then
            do ia = 1, d_n_neigh
              local_psb3_deriv(2*pow_so3_dim+1:3*pow_so3_dim,  ia, 1:3) = tmp_pow_so3_deriv_out_w_3ch(1:pow_so3_dim, ia, 1:3)!*factor_ja_3ch
              local_psb3_deriv(2*pow_so3_dim+1:3*pow_so3_dim,  0, 1:3) = local_psb3_deriv(2*pow_so3_dim+1:3*pow_so3_dim, 0, 1:3) - tmp_pow_so3_deriv_out_w_3ch(1:pow_so3_dim, ia, 1:3)!*factor_ja_3ch
            end do
          end if
        end if
      end if
