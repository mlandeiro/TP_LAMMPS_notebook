module milady_data
   ...
   subroutine allocate_local_desc
      use temporary_data_cov, only : dim_xdesc, dim_xdesc_quadratic, dim_xdesc_kernel, dim_xdesc_quadratic_only, dim_xdesc_kernel_only
      ...
      if (allocated(local_desc_deriv)) deallocate(local_desc_deriv) ; allocate(local_desc_deriv(dim_xdesc, 0:imm_neigh, 3))
      if (snap_order == snap_quadratic) then
            ...
            if(allocated(q_local_desc_deriv)) deallocate(q_local_desc_deriv) ; allocate(q_local_desc_deriv(dim_xdesc_quadratic_only, 3))
      end if

      if (snap_order == snap_kernel) then
            if (allocated(local_desc_kernel))       deallocate(local_desc_kernel)       ; allocate(local_desc_kernel(dim_xdesc_kernel_only))
            if (allocated(local_desc_kernel_deriv)) deallocate(local_desc_kernel_deriv) ; allocate(local_desc_kernel_deriv(dim_xdesc_kernel_only, 0:imm_neigh, 3))
      end if
