...
use ml_in_lammps_module, ONLY:  imm_neigh, weighted, weighted_3ch, lbso4_diag, jj_max, bisso4_l, bisso4_l1, bisso4_l2, bisso4_dim, cg_vector
implicit none
...
do j= 0, jj_max ; do m1 = -j,j,2; do m2 = -j,j,2
  if (allocated(class_mml(m2,m1,j)%vecall)) deallocate(class_mml(m2,m1,j)%vecall) ; allocate(class_mml(m2,m1,j)%vecall(1:3*imm_neigh))
end do ; end do ; end do
