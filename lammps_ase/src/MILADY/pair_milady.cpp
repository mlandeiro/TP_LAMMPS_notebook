...
PairMILADY::~PairMILADY()
{
  ...
  if(MiladyParams) {
    ...
    // ... COS            //
    memory->destroy(MiladyParams->weights_3ch);
    // ... COS            //
    memory->destroy(MiladyParams->ref_energy_pa);
    delete MiladyParams;
  }
  if (allocated) {
...
void PairMILADY::compute(int eflag, int vflag)
{
  ...
  int i,j,ii,jj, n,inum_half,inum_full,jnum_half, jnum_full, errorflag;
  int *ilist_half,*jlist_half, *numneigh_half,**firstneigh_half;
  int *ilist_full,*jlist_full, *numneigh_full,**firstneigh_full;
  ...
  // std::cout << inum_full << "inum_full" << std::endl ; //
  // std::cout << ilist_full << "ilist_full" << std::endl ; //
  ...
  for (ii = 0; ii < inum_half; ii++) {
    ...
    ifort = i+1;
  /*   itype = type[i]; */
    jnum_full = numneigh_full[i] ;
    jlist_full = firstneigh_full[i] ;
    /*for (jj=0; jj < jnum_full ; jj++ ){
      j = jlist_full[jj] ;
      std:cout << "ll " << i << " " << j << " " << type[i]<<" "<< type[j] << std::endl ;
    }

    // MILADY computation of descriptors and thus forces //
     std::cout << "locall " << i << " type "  << type[i] << " ntype " << ntype << std::endl ; */
  ...
  for (ii = 0; ii < inum_half; ii++) {
    i = ilist_half[ii];
    std::cout << "fll " << i << " "  << '1' << " " << f[i][0] << std::endl ;
    std::cout << "fll " << i << " "  << '2' << " " << f[i][1] << std::endl ;
    std::cout << "fll " << i << " "  << '3' << " " << f[i][2] << std::endl ;
  }
  */
  ...
  for (ii = 0; ii < inum_half; ii++) {
    i = ilist_half[ii];
    std::cout << "cll " << i << " "  << '1' << " " << f[i][0] << std::endl ;
    std::cout << "cll " << i << " "  << '2' << " " << f[i][1] << std::endl ;
    std::cout << "cll " << i << " "  << '3' << " " << f[i][2] << std::endl ;
  }
  */
...
{
  /* std::cout << "PairMILADY::coeff"  << std::endl ; */
  int i,j,m,n;
  ...
  */
  ...
  /* std::cout << "pair_milady Reading " << arg[2] << std::endl ; */
  read_file(arg[2]);
  /* std::cout << "MiladyStruct Data:"  << std::endl ; */
  milady_setup_(&MiladyParams->nochannels,&MiladyParams->nelements,&MiladyParams->ninteractions,
                &MiladyParams->nparameters,&MiladyParams->parameters[0],
                &MiladyParams->rcutvec[0],
                &MiladyParams->ndetails, &MiladyParams->details[0],
                &MiladyParams->weights[0],  &MiladyParams->weights_3ch[0], &MiladyParams->ref_energy_pa[0],
                &MiladyParams->descriptor_type);
  // read args that map atom types to elements in potential file
  // map[i] = which element the Ith atom type is, -1 if NULL

  /* std::cout << "NARG" << narg  << std::endl ; */
  // this is for the line pair_style >= 3 means the elements //
  for (i = 3; i < narg; i++) {
  /* std::cout << narg << ' ' << i << ' '<< arg[i]  << std::endl ; */
    ...
    else error->all(FLERR,"MILADY: pair_style error: no matching element in MILADY potential file");
  }
...
double PairMILADY::init_one(int i, int j)
{
  ...
  if(ij >= MiladyParams->ninteractions) ij = MiladyParams->ninteractions-1;
...
void PairMILADY::read_file(char *filename)
{
  ...
  if(comm->me == 0) {
    ...
    try {
      ...
      file->nochannels = values.next_int();
      /* std::cout << file->nochannels << "...CHANN3L NO 1..." << std::endl ; */
      file->nelements = values.next_int();
      /* std::cout << file->nochannels << "...CHANN3L NO 2..." << file->nelements << std::endl ; */
      // TOM file->ninteractions = ( file->nelements * (file->nelements+1) ) / 2; //
      // COS in new version nineractions is always 1 //
      file->ninteractions = 1 ;

      //values = reader.next_values(2*file->nelements);

      // extract element names from nelements line
      if (file->nochannels==1 || file->nochannels==2){
        if ((int)values.count() != 4*file->nelements+2)
        ...
        // COS 3ch is created even for this case even the values will be 1 //
        memory->create(file->weights_3ch,file->nelements,"pair:weights_3ch");
        //
        memory->create(file->ref_energy_pa,file->nelements,"pair:ref_energy_pa");
        ...
        for (int i = 0; i < file->nelements; i++) {
          ...
          file->weights[i] = w;
          // is just fake reading ... now the format is fix //
          const double w3 = values.next_double();
          file->weights_3ch[i] = 1.0;
          // COS reference energy for alloys ..... //
          const double w4 = values.next_double();
          file->ref_energy_pa[i] = w4;

        }
      }

      if (file->nochannels==3){
        if ((int)values.count() != 4*file->nelements+2)
          error->one(FLERR,"Incorrect number for 3 channels version of element names,weights_1, weights_2 in MILADY potential file");
      ...
      /* std::cout << file->nochannels << "...CHANN3L NO 3..."  << std::endl ; */
      ...
      /* DCD memory->create(file->parameters,file->ninteractions*file->nparameters+1,"pair:parameters"); */
      memory->create(file->parameters,file->ninteractions*file->nparameters,"pair:parameters");
      ...
      for (int i = 0; i < file->ninteractions; i++) {
        ...
        std::cout<<"rcutvec["<<i<<"] = "<<file->rcutvec[i]<<std::endl;
      }
  ...
  // ... COS            ///
  MPI_Bcast(&file->nochannels, 1, MPI_INT, 0, world);
  ...
  // COS .. //
  MPI_Bcast(&file->weights_3ch[0],file->nelements,MPI_DOUBLE,0,world);
  // COS ..//
  MPI_Bcast(&file->ref_energy_pa[0],file->nelements,MPI_DOUBLE,0,world);
  MPI_Bcast(&file->details[0],file->ndetails,MPI_DOUBLE,0,world);
  MPI_Bcast(&file->rcutvec[0],file->ninteractions,MPI_DOUBLE,0,world);
  /* DCD MPI_Bcast(&file->parameters[0],file->ninteractions*file->nparameters+1,MPI_DOUBLE,0,world); */
  MPI_Bcast(&file->parameters[0],file->ninteractions*file->nparameters,MPI_DOUBLE,0,world);
}
